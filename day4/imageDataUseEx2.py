...
with torch.no_grad(): # 이 영역을 미분추적하지 않도록
    x_test = test_set.test_data.view(-1, 28*28).float()
    y_test = test_set.test_labels

    prediction = model(x_test)
    correction_prediction = torch.argmax(prediction, dim=1) == y_test # torch.argmax(prediction, dim=1)-확률이 가장 높은 것을 정답으로 쓰겠다, 그리고 그걸 y_test랑 비교
